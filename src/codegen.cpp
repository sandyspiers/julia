...
static jl_cgval_t emit_invoke(jl_codectx_t &ctx, const jl_cgval_t &lival, ArrayRef<jl_cgval_t> argv, size_t nargs, jl_value_t *rt, Value *age_ok, bool always_inline);
...
static jl_cgval_t emit_invoke(jl_codectx_t &ctx, jl_expr_t *ex, jl_value_t *rt)
{
    ...
    return emit_invoke(ctx, lival, argv, nargs, rt, nullptr, false);
}
...
static Function *emit_modifyhelper(jl_codectx_t &ctx2, const jl_cgval_t &op, const jl_cgval_t &modifyop, jl_value_t *jltype, Type *elty, jl_cgval_t rhs, const Twine &fname, bool gcstack_arg)
{
    ...
    jl_cgval_t ret = emit_invoke(ctx, modifyop, argv, 3, (jl_value_t*)jl_any_type, nullptr, true);
    ...
    jl_unionall_t *unionall_env, jl_svec_t *sparam_vals, jl_array_t **closure_types)
{
    ...
    jl_cgval_t retval;
    if (codeinst) {
        retval = emit_invoke(ctx, mark_julia_const(ctx, (jl_value_t*)codeinst), inputargs, nargs + 1, astrt, age_ok, false);
        ...
        jl_codegen_params_t &params)
{
    ...
    if (specsig && JL_FEAT_TEST(ctx, gcstack_arg)) {
        ...
        ++AI;
        AttrBuilder param(ctx.builder.getContext());
        attrs[gcstackarg->getArgNo()] = AttributeSet::get(gcstackarg->getContext(), param);
        //ctx.pgcstack = gcstackarg;
    }
    allocate_gc_frame(ctx, b0);
    Value *last_age = NULL;
